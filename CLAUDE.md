...
**Message resolution**: a repository whose `.gg/publish_config.json` carries a `nextCommitMessage` is asked **separately**, with that proposal pre-filling its editor — the AI keeps it up to date with whatever is currently uncommitted there, so the default always describes the pending work. Its `firstLine` becomes the commit's first line (at most `maxCommitMessageFirstLineLength` = 60 characters; a longer one re-opens the editor with what was typed, up to three times), its `details` the body. After a **successful** commit the message is appended to that repository's `commits` — deduplicated by a deep JSON comparison (`gg_json`'s `deeplEquals`), so a re-run adds nothing twice — while `nextCommitMessage` deliberately stays: it is a standing proposal, not a buffer that committing empties. A failed commit records nothing. Repositories without a proposal share the one ticket-wide message, resolved once: an explicit `-m`/`--message` is used as-is. Without one, the ticket description — read from the root `ticket.json` via `readTicketDescription` (gg_multi_core) — seeds the interactive editor (`interact`'s `Input`, guarded by gg_one's `throwWhenNotATerminal` so headless runs fail fast). The edited text wins; clearing it falls back to the description. When nothing resolves, `null` is forwarded and gg_one decides — it only demands a message from repos that actually have something to commit. The prompt is skipped for an empty ticket and whenever a message was passed, so `do commit -m …` stays non-interactive.
...
**A state that was pushed already is skipped.** Step 9 records the ticket hash as `doPush` in `<ticket>/.gg.json`; the read happens before step 1, so a second run on an unchanged ticket returns with »Already pushed« instead of merging, upgrading and verifying again. The hash is written *after* the flow, because the merge, the upgrade and its system commit are what the remote now holds. Two separate flags: **`--force`/`-f` bypasses the recorded state** (it does *not* force-push), **`--git-force` force-pushes** — that flag was called `--force` before this split. Caveat of the skip: when `origin/main` moves while the ticket's tree stays unchanged, the feature branches no longer pick it up until something local changes; `--force` is the way to merge main in again.

1. **Uncommitted-changes check** across all repos, before anything is merged (gg_git's `IsCommitted` per repo).
2. **Merge main into the feature branches**: per repo `git fetch origin <main>` + `git merge origin/<main>` (`<main>` detected via gg_publish's `MainBranch`: the branch the remote declares as `origin/HEAD`, else `main`/`master`; a repo with none is skipped). Runs for *every* repo before *any* repo is pushed. **Merge conflicts** throw `MergeConflictException` whose message carries the full report (conflicting files + `gg do commit -m 'Merge <main>' --no-log`); the half-merged tree survives for the user to resolve, and `do review`/`do publish` rethrow the exception unwrapped. Any other merge failure runs `git merge --abort` and fails the push.
...
7. **Integrating the remote feature branch** (`_integrateRemoteBranch`, per repo, on the repo's *current* branch; skipped in detached HEAD): a branch that does not exist on the remote, or whose remote tip is contained in the local history, needs nothing. Otherwise the local branch is rebased onto it (`git pull --rebase`, never a force push; a rebase conflict aborts the rebase and fails with a manual hint) — _unless_ the remote branch is **obsolete**: that happens when a ticket branch was squash-merged into the default branch, the provider did not delete it, and the ticket is used again. `_remoteBranchIsObsolete` compares against the repo's default branch (`<main>` as detected by gg_publish's `MainBranch` — `origin/HEAD`, else `main`/`master` — so a repo whose default branch is `develop` works too; a repo without one falls back to the rebase) and says yes only when `origin/<main>` is contained in `HEAD` **and** every commit the remote branch holds on top of `HEAD` is either already on `origin/<main>` by content (`git cherry` compares patch ids, so a squash merge is recognized) or one of gg's own bookkeeping commits. A single real, unmerged commit makes it fall back to the rebase. An obsolete branch is overwritten with `git push --force-with-lease=<branch>:<remote head>` — the lease pins the hash the analysis was made from; a rejected push fails with a hint to delete the leftover branch manually.
8. **Push** via gg_one's `gg do push` per repo — `--git-force` arrives there as its `force`. Push/integration failures are collected per repo and summarized; merge, upgrade, can-commit and system-commit failures abort before any repo is pushed.
9. **Record the push**: `TicketState.writeSuccess` under `DoPushCommand.stateKey` (`'doPush'`), which is what the short-circuit above reads. A failed push writes nothing.
...
**A state that was reviewed already is skipped.** The recorded `didReview` hash is read right after the repos are collected — before the push, the plan and the pull requests — and a match ends the run with »Already reviewed. Review again with "gg do review --force"«, the same short-circuit `gg can commit` and `can review` have. `--force`/`-f` ignores the hash, runs every step and is handed on to `can review`, so its own cached success is bypassed too. `--reask-version` bypasses the skip as well — the question it wants asked lives in the plan, which a short-circuited run never reaches.
