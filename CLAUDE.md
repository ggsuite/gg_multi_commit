...
6. **Commit the upgrade changes** as a gg system commit: per repo, when `IsCommitted` reports a dirty tree, gg_one's `gg do commit` runs with `#gg: dart pub upgrade [--major-versions ]--tighten` (`#gg: dart pub get` when the upgrade step was skipped) and `updateChangeLog: false`.
...
**The pull requests** (`_createPullRequests`, gg_one's `CreatePullRequest`): opened as soon as the branches are on the remote, with **no auto-merge flag** — that is `do publish`'s job. The title is the ticket description, else the ticket name; an already open pull request keeps its title. A provider that cannot be reached is reported per repo in yellow and the remaining repos are still processed — it never fails the review, and the flag is still written. The urls are collected first and printed afterwards (`GgStatusPrinter` overwrites its own line). The printed urls link **directly to the changes** (`_changesUrl`): GitHub gets `/changes` appended, Azure DevOps `?_a=files`.
